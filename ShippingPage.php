...
<body>
   ...
   <!------------End Header ------------>
	   ...
	   <form name = "myform" Action = "EditBill.php" method = "POST" >
			Customer ID <input name="Cusid" method = "post">
			Transaction ID <input name="Traid" method = "post">
			<button  name = "su"> Edit </button>
		</form>  
		...
		<?php 
			...
			$sql = "select Customer_ID , Transaction_ID from orderprocessing where processed_flag = 1 and shipped_flag = 0	";	
			$result = mysqli_query($con,$sql);
			while($strrow = mysqli_fetch_array($result)){ 
					?>
					<tr>
					<td><?php echo $strrow['Customer_ID'];?></td>
					<td><?php echo $strrow['Transaction_ID'];?></td>
					</tr>
				<?php					
			}
			?>
